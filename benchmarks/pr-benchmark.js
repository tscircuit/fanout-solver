...
export async function preparePrBenchmark({ github, context, core }) {
  ...
  if (rawNumber) {
    ...
    const comment = await github.rest.issues.createComment({
      ...
      body: `## Dataset 31 — AM62L, RK3308, K230, i.MX6ULL, and T113-S3 fanout benchmark\n\nQueued for \`${ref.slice(0, 7)}\` on Blacksmith. All 60 dataset-fanout31-am62l samples (12 AM62L, 12 RK3308, 12 K230, 12 i.MX6ULL, and 12 T113-S3) will run, with a per-sample deadline.\n\n[View run](${runUrl})`,
    })
...
export function renderBenchmarkComment(report, { ref, runUrl, result }) {
  ...
  header.push(
    "Only dataset-fanout31-am62l is benchmarked: 12 AM62L, 12 RK3308, 12 K230, 12 i.MX6ULL, and 12 T113-S3 cases. A case is solved only when all its SoC connections have validated fanout with the original constraints (135 for AM62L; 162 for RK3308; 171 for K230; 102 for i.MX6ULL; 128 for T113-S3). RAM fanout and inter-chip routing are separate phases.",
  )
  if (
