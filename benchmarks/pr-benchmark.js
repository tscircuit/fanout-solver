...
export async function preparePrBenchmark({ github, context, core }) {
  ...
  if (rawNumber) {
    ...
    const comment = await github.rest.issues.createComment({
      ...
      body: `## Dataset 31 — AM62L and RK3308 fanout benchmark\n\nQueued for \`${ref.slice(0, 7)}\` on Blacksmith. All 24 dataset-fanout31-am62l samples (12 AM62L and 12 RK3308) will run, with a per-sample deadline.\n\n[View run](${runUrl})`,
    })
...
export function renderBenchmarkComment(report, { ref, runUrl, result }) {
  ...
  header.push(
    "Only dataset-fanout31-am62l is benchmarked: 12 AM62L and 12 RK3308 cases. A case is solved only when all its SoC connections have validated fanout with the original constraints (135 for AM62L; 162 for RK3308). RAM fanout and inter-chip routing are separate phases.",
  )
  if (
