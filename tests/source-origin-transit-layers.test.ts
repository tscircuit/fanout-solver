...
test("source-origin routing uses permitted transit layers while the complete bus retains its assigned exits", async () => {
  ...
  const consume = (transitLayers: string[]) => {
    const steps = routeReservedViaBusesSteps({ ...params, transitLayers })
    let next = steps.next()
