...
test("repairs boundary links after a real transit via inside the repair strip", async () => {
  ...
  const original = connections.map((connection) => {
    const viaPoint = { x: 0, y: connection.sourcePoint.y },
      finalViaPoint = { x: -1.6, y: viaPoint.y },
    ...
    const tail = [finalViaPoint, { x: -1.96, y: viaPoint.y }, exitPoint]
    const segments = [
      ...prefix.segments,
  ...
  for (const [plans, offset, label] of [
    [original, 0, "Before: transit returns inside strip"],
    [repaired, 5, "After: original barrels retained"],
  ] as const) {
    graphics.texts!.push({
      x: -2 + offset,
      y: 0.65,
