...
import { routeLeftCrossbarBusSteps } from "lib/route-left-crossbar-bus"
...
test("uses only the allowed crossbar layer to preserve permuted corner targets", async () => {
  const layerNames = [
    "top",
    "inner1",
    "inner2",
  ...
  const run = (candidateBus: PreparedBus): FanoutRoutePlan[] | null => {
    const steps = routeLeftCrossbarBusSteps({ ...params, bus: candidateBus })
