...
test("repairs separated exit clusters on independent grid phases within one atomic bus", async () => {
  const traceWidth = 0.08128,
    clearance = 0.08128,
    viaDiameter = 0.04,
    viaHoleDiameter = 0.02,
    layerNames = ["top", "inner1", "bottom"]
  const sharedBoundary = { minX: -2, maxX: 2, minY: -4, maxY: 4 }
  const pads: Obstacle[] = Array.from(
    { length: 16 },
    (_, i) => ((i % 8) - 3.5) * 0.32512 + (i < 8 ? -1.5 : 1.5),
  ).map((y, i) => ({
    type: "rect",
    shape: "circle",
    center: { x: 0.4, y },
  ...
  const connections: PreparedConnection[] = pads.map((sourceObstacle, i) => {
    const sourcePoint = {
      ...
      targetPoint = {
        x: -3,
        y:
          (sourceObstacle.center.y - (i < 8 ? -1.5 : 1.5)) / 2 +
          (i < 8 ? -1.5 + 0.00206 : 1.5 + 0.02514),
        layer: "bottom",
      }
    return {
      connection: {
  ...
  const original = connections.map((connection) => {
    ...
    const tail = [
      finalViaPoint,
      {
        x: -1.96,
        y:
          (viaPoint.y - (connection.connectionIndex < 8 ? -1.5 : 1.5)) / 2 +
          (connection.connectionIndex < 8 ? -1.5 : 1.5),
      },
      exitPoint,
    ]
    const segments = [
  ...
  for (const [plans, offset, label] of [
    [original, 0, "Before: two disconnected exit corridors"],
    [repaired, 5, "After: independent exact phases, same bus"],
  ] as const) {
    graphics.texts!.push({
      x: -2 + offset,
      y: 3.3,
