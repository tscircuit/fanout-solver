...
const captured = JSON.parse(
  gunzipSync(
    readFileSync(
      new URL("./am62l-real-decoupling-fanout.json.gz", import.meta.url),
    ),
  ).toString("utf8"),
) as {
...
/** Reconstruct the exact solver input captured from the real Core TSX circuit. */
export const createAm62lRealDecouplingInput = () => {
  const inputSrj = {
    ...captured.inputSrj,
