...
test("source-layer transit crosses a blocked target layer while preserving fixed source and plane copper", async () => {
  ...
  expect(
    plans[0]!.segments.some(
      (segment) =>
        segment.layer === "top" &&
        Math.min(segment.start.x, segment.end.x) < -0.2 &&
        Math.max(segment.start.x, segment.end.x) > 0.2,
    ),
  ).toBe(true)
  ...
  const restricted = { ...bus, routableEscapeLayers: ["bottom"] }
  const forbidden = routeReservedViaBusesSteps({
    ...params,
    buses: [restricted],
    allBuses: allBuses.map((original) =>
      original.busId === bus.busId ? restricted : original,
    ),
  ...
  const sameLayerTarget = routeReservedViaBusesSteps({
    ...params,
    targetLayer: "top",
  })
  expect(sameLayerTarget.next()).toMatchObject({ done: true, value: null })
  await expect(
