...
test("repairs all 26 lanes in a connected boundary cluster without moving sources or exits", async () => {
  const traceWidth = 0.04,
    clearance = 0.04,
    viaDiameter = 0.08,
    viaHoleDiameter = 0.04,
    layerNames = ["top", "bottom"]
  const sharedBoundary = { minX: -2, maxX: 2, minY: -3, maxY: 3 }
  ...
  const bus: PreparedBus = {
    ...
    componentBounds: { minX: 0.36, maxX: 0.44, minY: -2.04, maxY: 2.04 },
    sharedBoundary,
    xCoordinates: [0.4],
  ...
  for (const [plans, offset, label] of [
    [original, 0, "Before: 26 crowded boundary links"],
