...
import { routeBottomCrossbarBusSteps } from "lib/route-bottom-crossbar-bus"
...
test("bottom crossbar permutes four lanes using only the bus permitted layers", async () => {
  const width = 0.08128,
    clearance = 0.08128,
    viaDiameter = 0.24,
  ...
  const steps = routeBottomCrossbarBusSteps(params)
  let step = steps.next()
  ...
  const restricted = routeBottomCrossbarBusSteps({
    ...params,
