...
test("shortened singleton routing becomes length-matchable without changing other nets", async () => {
  ...
  expect(phases.has("layer")).toBe(false)
  ...
  const invalidSource = rerouteBusWithRetainedBoundaryTailsSteps({
    ...rules,
    inputSrj,
    plans: repaired.map((plan) =>
      plan.busId === "plane" ? { ...plan, via: undefined } : plan,
    ),
    preparedBuses,
    layerNames,
  })
  let missing = invalidSource.next()
  while (!missing.done) missing = invalidSource.next()
