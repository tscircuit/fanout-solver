...
import { getCornerTargetTrack, routeBusAlternativesSteps } from "lib/route-bus"
...
test("moves an ordered pair into an interior gap after routing a plain-edge singleton", async () => {
  ...
  const recovered = finish(
    routeReservedNarrowBusesSteps({ ...params, buses: buses.slice(0, 2) }),
  )
  expect(recovered).toHaveLength(3)
  expect(recovered![0]!.busId).toBe("SINGLE")
